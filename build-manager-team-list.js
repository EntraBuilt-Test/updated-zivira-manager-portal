      ...
      <section className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/90 dark:border-slate-800 p-6 shadow-sm relative overflow-hidden" data-purpose="team-page-header">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5 relative z-10">
          <div>
            ...
            <h1 className="text-2xl lg:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight mt-2">&nbsp;Team Management</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3.5 mt-6 pt-6 border-t border-slate-100 dark:border-slate-800/80">
          <div className="bg-slate-50/70 dark:bg-slate-900/50 hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-xl p-4 border border-slate-200/70 dark:border-slate-800 transition-all flex flex-col justify-between">
            ...
            <div className="my-2 flex items-baseline gap-2">
              <span className="text-2xl lg:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">{team.length}</span>
          ...
          <div className="bg-emerald-50/50 dark:bg-emerald-900/20 hover:bg-emerald-50/70 dark:hover:bg-emerald-900/30 rounded-xl p-4 border border-emerald-200/80 dark:border-emerald-800/50 transition-all flex flex-col justify-between">
            ...
            <div className="my-2 flex items-baseline gap-2">
              <span className="text-2xl lg:text-3xl font-extrabold text-emerald-900 dark:text-emerald-400 tracking-tight">{activeCount} Reps</span>
            </div>
          ...
          <div className="bg-slate-50/70 dark:bg-slate-900/50 hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-xl p-4 border border-slate-200/70 dark:border-slate-800 transition-all flex flex-col justify-between">
            ...
            <div className="my-2 flex items-baseline gap-2">
              <span className="text-2xl lg:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">{Math.min(team.length, 4)}</span>
          ...
          <div className="bg-slate-50/70 dark:bg-slate-900/50 hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-xl p-4 border border-slate-200/70 dark:border-slate-800 transition-all flex flex-col justify-between">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Target Compliance</span>
              <span className="text-[10px] font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 px-1.5 py-0.5 rounded border border-emerald-200 dark:border-emerald-800">+4.8%</span>
            </div>
            <div className="my-2">
              <div className="flex items-baseline justify-between mb-1.5">
                <span className="text-2xl lg:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">94.2%</span>
                <span className="text-[10px] text-slate-400 font-semibold">Goal: 90%</span>
              </div>
              <div className="w-full bg-slate-200 dark:bg-slate-800 rounded-full h-1.5 overflow-hidden">
                <div className="bg-teal-600 dark:bg-teal-500 h-1.5 rounded-full" style={{ width: '94.2%' }}></div>
              </div>
            </div>
            <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">All territories exceeding quota</p>
          </div>
          ...
          <div className="bg-slate-50/70 dark:bg-slate-900/50 hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-xl p-4 border border-slate-200/70 dark:border-slate-800 transition-all flex flex-col justify-between">
            ...
            <div className="my-2 flex items-baseline gap-2">
              <span className="text-2xl lg:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">{team.length * 36}</span>
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400">HCPs</span>
            </div>
            <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium truncate">Assigned across {activeCount} active MRs</p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/90 dark:border-slate-800 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-200/80 dark:border-slate-800 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex items-center gap-1.5 p-1 bg-slate-100/90 dark:bg-slate-800 rounded-xl border border-slate-200/60 dark:border-slate-700 w-fit flex-wrap">
            <button className="px-3 py-1.5 text-xs font-bold rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm border border-slate-200/40 dark:border-slate-600">
            ...
            <button className="px-3 py-1.5 text-xs font-semibold rounded-lg text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-white/60 dark:hover:bg-slate-700 transition-colors">
          ...
          <div className="flex items-center gap-3 flex-1 max-w-xl justify-end">
            <div className="relative flex-1">
              ...
              <input className="w-full pl-9 pr-3.5 py-2 text-xs rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900 focus:bg-white dark:focus:bg-slate-800 focus:border-teal-600 focus:ring-2 focus:ring-teal-600/20 text-slate-800 dark:text-slate-200 transition-all placeholder:text-slate-400" placeholder="Search representative name, code..." type="text" />
            </div>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[950px]">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-xs font-medium">
              {!loading && team.length === 0 && (
                <tr>
                  <td colSpan={7} className="text-center py-8 text-slate-500 dark:text-slate-400">No team members found</td>
                </tr>
              )}
              {team.map((emp, i) => (
                <tr key={emp.id} className="hover:bg-teal-50/20 dark:hover:bg-teal-900/10 transition-colors group">
                  ...
                  <td className="py-3.5 px-4">
                    <button className="px-2.5 py-1 text-[11px] font-bold text-teal-800 dark:text-teal-400 hover:text-teal-900 dark:hover:text-teal-300 hover:bg-teal-50 dark:hover:bg-teal-900/50 rounded-lg transition-colors border border-transparent hover:border-teal-200 dark:hover:border-teal-800" title="View Profile">
                      View
                    </button>
                  </td>
                </tr>
...
fs.writeFileSync('C:/Users/ELCOT/Downloads/Zivira-Manager-main/components/manager-team-list.tsx', fileContent, 'utf8');
console.log('Successfully wrote manager-team-list.tsx');
